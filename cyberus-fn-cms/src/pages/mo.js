...
export default function CmsMoReport(){
...
console.log("PAYLOAD : ",(JSON.stringify(payload)))
    fetch(`${apiUrl}/api/user/mo-report`, {
      method: 'POST',
      headers: {
...
<br/>
   ...
   {/* --- Menu --- */}
      ...
      <div style={{marginTop:'10px', padding: '60px' ,backgroundColor: '#FFFFFF' , zIndex:'revert'}}>
             <center> <h1 className="text-3xl font-bold text-gray-800 ">Subscription Report</h1></center>
              {/* Date Picker UI */}
            ...
            <table className={styles.table}>
              <thead>
                <tr >
                  ...
                  <th><center>AIS-MO</center></th>
                  <th><center>DTAC-MO</center></th>
                  <th><center>TMVH-MO</center></th>
                </tr>
              </thead>
              <tbody>
                {data.map((row, index) => (
                  <tr key={index} className={`border-b border-gray-200 ${index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}`}>
                    <td><center>{row['mo-date']}</center></td>
                    <td><a href={`${currentUrl}/mo-detail?telco=ais&start=${row['start-timestamp']}&end=${row['end-timestamp']}`} target="_blank" rel="noopener noreferrer">{row['ais-mo'].toLocaleString()}</a></td>
                    <td><a href={`${currentUrl}/mo-detail?telco=dtac&start=${row['start-timestamp']}&end=${row['end-timestamp']}`} target="_blank" rel="noopener noreferrer">{row['dtac-mo'].toLocaleString()}</a></td>
                    <td><a href={`${currentUrl}/mo-detail?telco=tmvh&start=${row['start-timestamp']}&end=${row['end-timestamp']}`} target="_blank" rel="noopener noreferrer">{row['tmvh-mo'].toLocaleString()}</a></td>
                  </tr>
                ))}
              </tbody>
            </table>
